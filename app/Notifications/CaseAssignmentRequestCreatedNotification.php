...
class CaseAssignmentRequestCreatedNotification extends Notification implements ShouldQueue
{
    ...
    {
        return (new MailMessage)->subject('Yeni dosya atama/devir talebi')
            ->line($this->request->caseFile->case_no.' · '.$this->request->type->label())
    ...
    {
        return ['type' => 'case_assignment_requested', 'case_file_id' => $this->request->case_file_id, 'message' => 'Yeni '.$this->request->type->label().' oluşturuldu.', 'url' => route('assignment-requests.index')];
    }
