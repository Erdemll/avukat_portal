...
<body class="min-h-full antialiased" data-auth-user-id="{{ $user->id }}" data-messages-url="{{ route('messages.index') }}">
    <div class="min-h-svh lg:grid lg:grid-cols-[17.5rem_minmax(0,1fr)]">
        ...
        <div class="min-w-0">
            <header class="sticky top-0 z-40 border-b border-slate-200 bg-[#f4f3ef]">
                <div class="flex h-16 items-center justify-between gap-3 px-4 sm:px-6 lg:h-20 lg:px-8 xl:px-10">
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        <details class="group lg:hidden">
                            ...
                            <div class="fixed inset-x-0 bottom-0 top-16 z-50 touch-pan-y overflow-y-auto overscroll-y-contain border-t border-slate-200 bg-[#f4f3ef] px-4 pt-5 [-webkit-overflow-scrolling:touch] [padding-bottom:max(1.25rem,env(safe-area-inset-bottom))] sm:px-6">
            ...
            <main class="mx-auto w-full max-w-[96rem] px-4 py-5 sm:px-6 sm:py-7 lg:px-8 lg:py-8 xl:px-10">
                ...
                @endif
                @if(session('status'))
                    <div class="mb-5 flex items-start gap-3 rounded-lg border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm text-emerald-800" role="status"><svg class="mt-0.5 h-5 w-5 shrink-0" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M9 12.75 11.25 15 15 9.75M21 12a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z" /></svg><span>{{ session('status') }}</span></div>
                @endif
                @if($errors->any())
                    <div class="mb-5 flex items-start gap-3 rounded-lg border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-800" role="alert">
                        ...
                        <div>
                            <p class="font-semibold">İşlem tamamlanamadı.</p>
