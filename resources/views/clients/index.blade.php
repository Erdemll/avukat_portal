...
<x-layouts.app title="Müvekkiller">
    ...
    <form class="mt-6 flex flex-wrap gap-3 rounded-xl border border-slate-200 bg-white p-4 shadow-sm" method="GET" action="{{ route('clients.index') }}"><input name="search" value="{{ request('search') }}" placeholder="Ad, soyad veya şirket ara" class="min-w-64 flex-1 rounded-lg border-slate-300"><select name="status" class="rounded-lg border-slate-300"><option value="">Tüm durumlar</option><option value="active" @selected(request('status') === 'active')>Aktif</option><option value="inactive" @selected(request('status') === 'inactive')>Pasif</option></select><button class="rounded-lg bg-slate-900 px-4 py-2 text-sm font-semibold text-white">Filtrele</button></form>
    <div class="mt-6 grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
        @forelse($clients as $client)
            <a class="group rounded-xl border border-slate-200 bg-white p-5 shadow-sm transition hover:-translate-y-0.5 hover:border-indigo-200 hover:shadow-md" href="{{ route('clients.show', $client) }}"><div class="flex items-start justify-between gap-3"><div class="flex h-10 w-10 items-center justify-center rounded-lg bg-indigo-50 font-bold text-indigo-700">{{ Str::upper(Str::substr($client->party->display_name, 0, 1)) }}</div><span class="rounded-full px-2 py-1 text-xs font-medium {{ $client->status === 'active' ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-200 text-slate-600' }}">{{ $client->status === 'active' ? 'Aktif' : 'Pasif' }}</span></div><h2 class="mt-4 font-semibold text-slate-900 group-hover:text-indigo-700">{{ $client->party->display_name }}</h2><p class="mt-1 text-sm text-slate-500">{{ $client->party->type->label() }}</p><p class="mt-4 text-xs text-slate-500">{{ $client->party->phone ?: 'Telefon yok' }} · {{ $client->party->email ?: 'E-posta yok' }}</p></a>
        @empty
