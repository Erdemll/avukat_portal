...
<x-layouts.app :title="$caseFile->case_no">
    <div class="flex flex-wrap items-start justify-between gap-4">
        <div class="min-w-0">
            ...
            <h1 class="mt-2 break-words text-3xl font-bold tracking-tight text-slate-950">{{ $caseFile->title }}</h1>
            <p class="mt-2 break-words text-sm text-slate-500">Lider: {{ $leadAssignment?->lawyer?->name ?? 'Belirlenmedi' }} · Açılış: {{ $caseFile->opened_at->format('d.m.Y') }}</p>
        </div>
            ...
            <section id="evraklar" class="rounded-xl border border-slate-200 bg-white shadow-sm">
                    ...
                    @forelse($caseFile->documents as $document)
                        <div class="px-6 py-4">
                            <div class="flex flex-wrap items-start justify-between gap-3">
                                <div class="min-w-0">
                                    <p class="break-words font-medium text-slate-900">{{ $document->title ?: $document->original_name }}</p>
                                    <p class="mt-1 text-xs text-slate-500">{{ $document->folder?->name ?? 'Klasörsüz' }} · v{{ $document->currentVersion?->version_no ?? 1 }} · {{ $document->human_size }} @if($document->isUdf()) · UYAP UDF @endif</p>
                                </div>
                                <div class="flex flex-wrap gap-2">
                                    @if($document->isUdf())
                                        <a class="text-xs font-medium text-indigo-700" href="{{ route('documents.udf.show', $document) }}">Görüntüle</a>
                                        @can('editUdf', $document)
                                            <a class="text-xs font-medium text-slate-700" href="{{ route('documents.udf.edit', $document) }}">Düzenle</a>
                                        @endcan
                                        <a class="text-xs font-medium text-slate-700" href="{{ route('documents.udf.download', $document) }}">İndir</a>
                                    @else
                                        @if($document->currentVersion && in_array($document->currentVersion->mime_type, ['application/pdf', 'image/jpeg', 'image/png'], true))
                                            <a class="text-xs font-medium text-indigo-600" href="{{ route('document-versions.preview', $document->currentVersion) }}" target="_blank">Önizle</a>
                                        @endif
                                        <a class="text-xs font-medium text-slate-700" href="{{ route('documents.download', $document) }}">İndir</a>
                                    @endif
                                </div>
                            </div>
                            @can('uploadVersion', $document)
                                <form class="mt-3 flex flex-wrap gap-2" method="POST" enctype="multipart/form-data" action="{{ route('document-versions.store', $document) }}">
                                    @csrf
                                    <input type="file" name="file" required class="w-full min-w-0 flex-1 text-xs sm:w-auto" @if($document->isUdf()) accept=".udf" @endif>
                                    <input name="change_note" placeholder="Sürüm notu" class="rounded-lg border-slate-300 text-xs">
                                    <button class="rounded-lg bg-slate-800 px-3 py-1.5 text-xs font-semibold text-white">Yeni Sürüm</button>
                                </form>
                            @endcan
                        </div>
                    @empty
                ...
                @if($canManageDocuments)
                    <div class="grid gap-5 border-t border-slate-100 bg-slate-50 px-6 py-5 lg:grid-cols-[1fr_2fr]">
                        ...
                        <form class="grid min-w-0 gap-3 sm:grid-cols-2" method="POST" enctype="multipart/form-data" action="{{ route('case-files.documents.store', $caseFile) }}">@csrf<select name="folder_id" class="rounded-lg border-slate-300 text-sm"><option value="">Klasörsüz</option>@foreach($caseFile->documentFolders as $folder)<option value="{{ $folder->id }}">{{ $folder->name }}</option>@endforeach</select><select name="document_type" class="rounded-lg border-slate-300 text-sm"><option value="other">Diğer Evrak</option><option value="petition">Dilekçe</option><option value="evidence">Delil</option><option value="notice">Tebligat</option><option value="report">Rapor</option><option value="contract">Sözleşme</option><option value="invoice">Fatura</option><option value="enforcement">İcra Evrakı</option></select><input name="title" placeholder="Tek dosya için evrak başlığı" class="rounded-lg border-slate-300 text-sm sm:col-span-2"><input type="file" name="documents[]" multiple required class="w-full min-w-0 text-sm sm:col-span-2" accept=".pdf,.doc,.docx,.xls,.xlsx,.jpg,.jpeg,.png,.udf"><button class="rounded-lg bg-indigo-700 px-4 py-2 text-sm font-semibold text-white sm:col-span-2">Evrak Yükle</button></form>
                    </div>
                @endif
            </section>

            <section id="durusmalar" class="rounded-xl border border-slate-200 bg-white shadow-sm"><div class="flex flex-wrap items-center justify-between gap-2 border-b border-slate-100 px-4 py-4 sm:px-6"><h2 class="font-semibold text-slate-900">Duruşmalar</h2>@if($canManageOperations)<a class="text-sm font-medium text-indigo-600" href="{{ route('hearings.create', ['case_file' => $caseFile->id]) }}">Duruşma Ekle</a>@endif</div><div class="divide-y divide-slate-100">@forelse($caseFile->hearings as $hearing)<a class="flex flex-col items-start gap-2 px-4 py-4 hover:bg-slate-50 sm:flex-row sm:items-center sm:justify-between sm:gap-3 sm:px-6" href="{{ route('hearings.edit', $hearing) }}"><div class="min-w-0"><p class="break-words font-medium text-slate-900">{{ $hearing->title }}</p><p class="mt-1 break-words text-xs text-slate-500">{{ $hearing->court ?: 'Mahkeme belirtilmedi' }} · {{ $hearing->lawyer?->name ?? 'Avukat belirtilmedi' }}</p></div><time class="shrink-0 whitespace-nowrap text-sm font-semibold text-indigo-700">{{ $hearing->hearing_at->format('d.m.Y H:i') }}</time></a>@empty<p class="px-6 py-8 text-sm text-slate-500">Duruşma kaydı bulunmuyor.</p>@endforelse</div></section>

            <section id="sureler" class="rounded-xl border border-slate-200 bg-white shadow-sm"><div class="flex flex-wrap items-center justify-between gap-2 border-b border-slate-100 px-4 py-4 sm:px-6"><h2 class="font-semibold text-slate-900">Hukuki Süreler</h2>@if($canManageOperations)<a class="text-sm font-medium text-indigo-600" href="{{ route('deadlines.create', ['case_file' => $caseFile->id]) }}">Süre Ekle</a>@endif</div><div class="divide-y divide-slate-100">@forelse($caseFile->deadlines as $deadline)<a class="flex flex-col items-start gap-2 px-4 py-4 hover:bg-slate-50 sm:flex-row sm:items-center sm:justify-between sm:gap-3 sm:px-6" href="{{ route('deadlines.edit', $deadline) }}"><div class="min-w-0"><p class="break-words font-medium text-slate-900">{{ $deadline->title }}</p><p class="mt-1 break-words text-xs text-slate-500">{{ $deadline->assignedLawyer?->name ?? 'Sorumlu belirtilmedi' }} · {{ $deadline->status->label() }}</p></div><time class="shrink-0 whitespace-nowrap text-sm font-semibold {{ $deadline->status === App\DeadlineStatus::Open && $deadline->due_at->isPast() ? 'text-red-700' : 'text-amber-700' }}">{{ $deadline->due_at->format('d.m.Y H:i') }}</time></a>@empty<p class="px-6 py-8 text-sm text-slate-500">Hukuki süre kaydı bulunmuyor.</p>@endforelse</div></section>

            <section id="gorevler" class="rounded-xl border border-slate-200 bg-white shadow-sm"><div class="flex flex-wrap items-center justify-between gap-2 border-b border-slate-100 px-4 py-4 sm:px-6"><h2 class="font-semibold text-slate-900">Görevler</h2><a class="text-sm font-medium text-indigo-600" href="{{ route('legal-tasks.create', ['case_file' => $caseFile->id]) }}">Görev Ekle</a></div><div class="divide-y divide-slate-100">@forelse($caseFile->tasks as $task)<a class="flex flex-col items-start gap-2 px-4 py-4 hover:bg-slate-50 sm:flex-row sm:items-center sm:justify-between sm:gap-3 sm:px-6" href="{{ route('legal-tasks.edit', $task) }}"><div class="min-w-0"><p class="break-words font-medium text-slate-900">{{ $task->title }}</p><p class="mt-1 break-words text-xs text-slate-500">{{ $task->assignee->name }} · {{ $task->status->label() }}</p></div><span class="shrink-0 text-sm text-slate-600">{{ $task->due_at?->format('d.m.Y H:i') ?? 'Tarihsiz' }}</span></a>@empty<p class="px-6 py-8 text-sm text-slate-500">Görev bulunmuyor.</p>@endforelse</div></section>

            <section id="tebligatlar" class="rounded-xl border border-slate-200 bg-white shadow-sm"><div class="flex flex-wrap items-center justify-between gap-2 border-b border-slate-100 px-4 py-4 sm:px-6"><h2 class="font-semibold">Tebligatlar</h2><a class="text-sm font-medium text-indigo-600" href="{{ route('service-notices.create',['case_file'=>$caseFile->id]) }}">Tebligat Ekle</a></div><div class="divide-y divide-slate-100">@forelse($caseFile->serviceNotices as $notice)<a class="flex flex-col gap-1 px-4 py-4 sm:flex-row sm:justify-between sm:gap-3 sm:px-6" href="{{ route('service-notices.edit',$notice) }}"><span class="min-w-0 break-words font-medium">{{ $notice->type->label() }}</span><span class="shrink-0 text-sm text-rose-700">{{ $notice->service_date->format('d.m.Y') }}</span></a>@empty<p class="px-6 py-8 text-sm text-slate-500">Tebligat kaydı yok.</p>@endforelse</div></section>

            <section class="rounded-xl border border-slate-200 bg-white shadow-sm"><div class="flex flex-wrap items-center justify-between gap-2 border-b border-slate-100 px-4 py-4 sm:px-6"><h2 class="font-semibold">Arabuluculuk</h2><a class="text-sm font-medium text-indigo-600" href="{{ route('mediations.create',['case_file'=>$caseFile->id]) }}">Kayıt Ekle</a></div><div class="divide-y divide-slate-100">@forelse($caseFile->mediations as $mediation)<a class="flex flex-col gap-1 px-4 py-4 sm:flex-row sm:justify-between sm:gap-3 sm:px-6" href="{{ route('mediations.edit',$mediation) }}"><span class="min-w-0 break-words">{{ $mediation->mediation_file_no ?: 'Arabuluculuk süreci' }}</span><span class="shrink-0 text-sm text-teal-700">{{ $mediation->status->label() }}</span></a>@empty<p class="px-6 py-8 text-sm text-slate-500">Arabuluculuk kaydı yok.</p>@endforelse</div></section>

            <section id="finans" class="rounded-xl border border-slate-200 bg-white shadow-sm"><div class="flex flex-wrap items-center justify-between gap-2 border-b border-slate-100 px-4 py-4 sm:px-6"><h2 class="font-semibold">Tahsilat & Ödeme</h2><a class="text-sm font-medium text-indigo-600" href="{{ route('financial-entries.index',['case_file'=>$caseFile->id]) }}">Tüm Hareketler</a></div><div class="divide-y divide-slate-100">@forelse($caseFile->financialEntries as $entry)<div class="flex flex-col gap-1 px-4 py-4 sm:flex-row sm:justify-between sm:gap-3 sm:px-6"><div class="min-w-0"><p class="font-medium">{{ $entry->type->label() }}</p><p class="break-words text-xs text-slate-500">{{ $entry->description }}</p></div><strong class="shrink-0">{{ number_format((float)$entry->amount,2,',','.') }} {{ $entry->currency }}</strong></div>@empty<p class="px-6 py-8 text-sm text-slate-500">Finans hareketi yok.</p>@endforelse</div></section>
